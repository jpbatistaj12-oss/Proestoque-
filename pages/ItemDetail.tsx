...
const ItemDetail: React.FC<ItemDetailProps> = ({ itemId, companyId, onBack, onUpdate }) => {
  ...
  useEffect(() => {
    ...
    setItem(currentItem);
    if (currentItem) {
      // Por padrão, sugere a sobra como o tamanho atual da chapa
  ...
  if (!item) return <div className="p-20 text-center font-black text-slate-400 uppercase tracking-widest">Localizando Material...</div>;
  ...
  const handleRegisterUsage = () => {
    ...
    let areaUsed = (item.width * item.height) / 10000; // m² total da peça original
    ...
    if (usageType === 'BAIXA') {
      ...
      // Registro de Corte (Sobra)
      if (leftoverWidth <= 0 || leftoverHeight <= 0 || cutWidth <= 0 || cutHeight <= 0) {
        return alert("Informe as medidas da peça que saiu (venda) e da sobra que retornou ao cavalete.");
      }
      newWidth = leftoverWidth;
      newHeight = leftoverHeight;
      newStatus = StockStatus.COM_SOBRA;
      // Área USADA agora é a área da peça que SAIU
    ...
    const newRecord: CutHistoryRecord = {
      ...
      observations: observations + (usageType === 'SOBRA' ? ` (Sobra: ${newWidth}x${newHeight}cm | Cortado: ${cutWidth}x${cutHeight}cm)` : ''),
      operatorName: user?.name || 'Operador',
      areaUsed: areaUsed, // Se for sobra, registra a área do que SAIU
    ...
    const updatedItem: InventoryItem = {
      ...
      width: newWidth,
      height: newHeight,
      availableArea: (newWidth * newHeight) / 10000, // Nova área disponível baseada na SOBRA
      status: newStatus,
    ...
    <div className="pb-10 animate-fadeIn max-w-7xl mx-auto px-4">
      <div className="flex justify-between items-center mb-8">
        <button onClick={onBack} className="flex items-center gap-2 text-slate-500 hover:text-slate-900 transition-colors">
          <ArrowLeft size={20} /> <span className="font-black text-[10px] uppercase tracking-widest">Painel de Controle</span>
        </button>
        <div className="flex items-center gap-3">
           <div className="bg-white px-4 py-2 rounded-xl border border-slate-200 shadow-sm font-black text-slate-900 text-xs uppercase">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 space-y-8">
            ...
            <div className="flex-1 p-10 space-y-8">
              ...
              <div className="grid grid-cols-2 gap-6 pt-6 border-t border-slate-100">
                 <div className="bg-slate-50 p-6 rounded-3xl text-center border border-slate-100">
                    <p className="text-[9px] text-slate-400 font-black uppercase tracking-widest mb-1">Unidades</p>
                    <p className={`text-4xl font-black ${item.quantity <= 0 ? 'text-red-500' : 'text-slate-900'}`}>{item.quantity}</p>
                 </div>
                 <div className="space-y-3 pt-2">
                    <div className="flex items-center gap-2">
                      <Ruler size={14} className="text-blue-500" />
                      <p className="text-xs font-bold text-slate-600">Medida Atual: {item.width}x{item.height}cm</p>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 text-slate-400">
                      <MapPin size={14} />
                      <p className="text-xs font-bold uppercase">{item.location || 'Local não definido'}</p>
                    </div>
                 </div>
              </div>
            </div>
          ...
          <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100">
             <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight flex items-center gap-3 mb-8"><History size={24} className="text-blue-500" /> Histórico de Cortes</h3>
             <div className="space-y-4">
               {item.history.length > 0 ? item.history.map(log => (
                 <div key={log.id} className="flex items-center gap-6 p-5 bg-slate-50 rounded-3xl border border-slate-100 group hover:bg-white hover:shadow-lg transition-all">
                    ...
                    <div className="text-right">
                       <p className="text-lg font-black text-slate-900">{log.type === 'ENTRADA' ? `+${log.quantityChange}` : (log.type === 'SOBRA' ? 'SOBRA' : 'BAIXA')}</p>
                 ...
                 <div className="py-12 text-center opacity-30">
                    ...
                    <p className="font-black uppercase text-[10px] tracking-widest">Sem movimentações registradas</p>
                 </div>
               )}
             </div>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[250] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-lg p-10 shadow-2xl space-y-6 animate-popIn max-h-[95vh] overflow-y-auto scrollbar-hide">
            <div className="flex justify-between items-center">
               <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">{usageType === 'BAIXA' ? 'BAIXA INTEGRAL' : 'REGISTRAR CORTE'}</h3>
            ...
            <div className="space-y-5">
               <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input type="text" placeholder="Nome" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all" value={client} onChange={e => setClient(e.target.value)} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Projeto/Obra</label>
                    <input type="text" placeholder="Ex: Cozinha" className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none focus:ring-4 focus:ring-blue-500/5 transition-all" value={project} onChange={e => setProject(e.target.value)} />
                  </div>
               </div>
               ...
               {usageType === 'SOBRA' && (
                 <div className="space-y-5">
                    <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-200 space-y-4 shadow-inner">
                       <p className="text-[10px] font-black text-slate-500 uppercase flex items-center gap-2"><ShoppingBag size={14} className="text-blue-500"/> PEÇA QUE SAIU (Venda)</p>
                       <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-6 bg-blue-50 rounded-[2rem] border border-blue-200 space-y-4 shadow-sm">
                       <p className="text-[10px] font-black text-blue-600 uppercase flex items-center gap-2"><Scissors size={14} className="text-blue-600"/> SOBRA QUE FICOU (Estoque)</p>
                       <div className="grid grid-cols-2 gap-4">
                          <input type="number" placeholder="Largura (cm)" className="w-full p-4 bg-white border border-blue-200 rounded-xl font-black text-blue-700" value={leftoverWidth || ''} onChange={e => setLeftoverWidth(Number(e.target.value))} />
                          <input type="number" placeholder="Altura (cm)" className="w-full p-4 bg-white border border-blue-200 rounded-xl font-black text-blue-700" value={leftoverHeight || ''} onChange={e => setLeftoverHeight(Number(e.target.value))} />
                       </div>
                       <p className="text-[9px] text-blue-400 font-bold text-center uppercase">Informe as medidas da peça que retorna ao cavalete</p>
                    </div>
                 </div>
               )}

               <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Observações do Corte</label>
                  <textarea placeholder="Detalhes técnicos..." className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold h-20 resize-none outline-none focus:ring-4 focus:ring-blue-500/5 transition-all" value={observations} onChange={e => setObservations(e.target.value)}></textarea>
               </div>
      ...
      {showRestockModal && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-[250] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] w-full max-w-md p-10 shadow-2xl space-y-6 animate-popIn">
            <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Entrada Manual</h3>
            <div className="space-y-6 text-center">
               ...
               <p className="text-[10px] text-emerald-600 font-black uppercase tracking-widest">Quantas chapas integrais estão entrando?</p>
      ...
      {showImageZoom && (
        <div className="fixed inset-0 bg-slate-950/95 z-[500] flex items-center justify-center p-4" onClick={() => setShowImageZoom(false)}>
          <img src={item.photos[0]} className="max-w-full max-h-full rounded-3xl shadow-2xl border-4 border-white/10" alt="" />
        </div>
      )}
    </div>
  );
